                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('guide.profile.update') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="daily_rate" class="form-control" value="{{ $profile->daily_rate }}" required min="100000">
                            </div>

                            <div class="mb-3">
                                <label>Maksimal Wisatawan</label>
                                <input type="number" name="max_travelers" class="form-control" value="{{ $profile->max_travelers }}" required min="1" max="20">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="languages[]" class="form-control" multiple required>
                                    @foreach(['Indonesia', 'English', 'Japanese', 'Mandarin', 'Korean', 'Arabic'] as $lang)
                                        <option value="{{ $lang }}" {{ in_array($lang, $profile->languages ?? []) ? 'selected' : '' }}>
                                            {{ $lang }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="skills[]" class="form-control" multiple required>
                                    @foreach(['Hiking', 'Photography', 'Cultural Tour', 'Food Tour', 'City Walk', 'History', 'Adventure', 'Family Tour'] as $skill)
                                        <option value="{{ $skill }}" {{ in_array($skill, $profile->skills ?? []) ? 'selected' : '' }}>
                                            {{ $skill }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="experience" class="form-control" value="{{ $profile->experience }}" placeholder="Contoh: 5 tahun memandu wisata di Bali">
                            </div>
